...
const financeRejectColumns = [
  {
    ...
    render: (value) => (
      <span className="font-mono font-semibold text-blue-700">
        FRJ-{value.toString().padStart(6, '0')}
      </span>
    ),
  },
  {
...
const createFinanceRejectData = (count) => {
  const employees = [
    'Juan Dela Cruz',
    'Maria Santos',
    'John Smith',
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      rejectedBy: `Finance Officer ${Math.floor(Math.random() * 5) + 1}`,
      rejectionReason: [
      ...
      notes: 'Review rejection reason for possible appeal or resubmission',
    }
  })
}
...
const FinanceReject = () => {
  ...
  const financeRejectData = useMemo(() => createFinanceRejectData(35), [])

  //   filter
  const filteredAndSortedData = useMemo(() => {
    let filtered = [...financeRejectData]
    ...
    return filtered.sort((a, b) => {
      if (sortKey === 'liquidationDate' || sortKey === 'rejectionDate') {
  ...
  }, [financeRejectData, sortKey, sortDirection, departmentFilter, reasonFilter, searchQuery])
  ...
  const handleExportSelected = () => {
    ...
    const selectedData = financeRejectData.filter((row) => selectedRows.includes(row.id))

    const exportData = selectedData.map((row) => ({
      'Reference ID': `FRJ-${row.referenceId.toString().padStart(6, '0')}`,
    ...
    link.setAttribute('download', `finance-reject-${new Date().toISOString().split('T')[0]}.csv`)
      ...
      <div className="flex-1 min-h-0 p-3">
        ...
        <div className="bg-component shadow-lg rounded-lg border border-slate-400 mb-3">
          <div className="px-4 py-1">
            <h1 className="text-2xl font-bold text-gray-800">Finance Rejected Liquidations</h1>
            <p className="text-gray-600">View liquidations rejected by finance department</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-4 pb-2 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {Array.from(new Set(financeRejectData.map((item) => item.department))).map(
                  (dept) => (
