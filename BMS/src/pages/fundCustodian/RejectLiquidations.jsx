...
const rejectLiquidationsColumns = [
  {
    key: 'referenceId',
    label: 'Reference ID',
    sortable: true,
...
const createRejectLiquidationsData = (count) => {
  const employees = [
    'Juan Dela Cruz',
    'Maria Santos',
    'John Smith',
...
const RejectLiquidations = () => {
  ...
  const rejectData = useMemo(() => createRejectLiquidationsData(25), [])

  // Filter and sort
  const filteredAndSortedData = useMemo(() => {
    let filtered = [...rejectData]
    ...
    return filtered.sort((a, b) => {
      if (sortKey === 'liquidationDate' || sortKey === 'resubmissionDate') {
  ...
  }, [rejectData, sortKey, sortDirection, departmentFilter, reasonFilter, searchQuery])
  ...
  const handleExportSelected = () => {
    ...
    const selectedData = rejectData.filter((row) => selectedRows.includes(row.id))
    ...
    link.setAttribute(
      'download',
      `reject-liquidations-${new Date().toISOString().split('T')[0]}.csv`
    )
      ...
      <div className="flex-1 min-h-0 p-3">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-4 pb-2 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {Array.from(new Set(rejectData.map((item) => item.department))).map((dept) => (
              ...
              >
                ...
                {Array.from(new Set(rejectData.map((item) => item.rejectionReason))).map(
                  (reason) => (
        ...
        <div className="h-[calc(100vh-480px)] lg:h-[calc(100vh-450px)] xl:h-[calc(100vh-360px)] overflow-hidden">
          <div className="bg-component shadow-lg rounded-lg border border-slate-400 h-full flex flex-col p-2">
            ...
            <PlatformTable
              columns={rejectLiquidationsColumns}
...
export default RejectLiquidations
