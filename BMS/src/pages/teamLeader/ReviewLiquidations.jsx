...
const reviewLiquidationsColumns = [
  {
    key: 'referenceId',
    label: 'Reference ID',
    sortable: true,
    ...
    render: (value) => (
      <span className="font-mono font-semibold text-blue-700">
        REV-{value.toString().padStart(6, '0')}
      </span>
    ),
  },
  {
...
const createReviewLiquidationsData = (count) => {
  const employees = [
    'Juan Dela Cruz',
    'Maria Santos',
    'John Smith',
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const liquidationDate = generateRandomDate(7)

    return {
      id: i + 1,
      referenceId: 1200000 + i + 1,
      ...
      reviewPriority: ['High', 'Medium', 'Low'][Math.floor(Math.random() * 3)],
      reviewer: Math.random() > 0.4 ? `Reviewer ${Math.floor(Math.random() * 5) + 1}` : '',
      reviewDeadline: generateRandomDate(14),
      documentStatus: ['Complete', 'Incomplete', 'Pending Verification'][
        Math.floor(Math.random() * 3)
      ],
      notes: ['Awaiting review', 'Need clarification', 'Pending approval', 'Under evaluation'][
...
const ReviewLiquidations = () => {
  ...
  const reviewData = useMemo(() => createReviewLiquidationsData(30), [])
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    if (priorityFilter) {
      filtered = filtered.filter((item) => item.reviewPriority === priorityFilter)
    }
    ...
    return filtered.sort((a, b) => {
      if (sortKey === 'liquidationDate' || sortKey === 'reviewDeadline') {
  ...
  }, [reviewData, sortKey, sortDirection, departmentFilter, priorityFilter, searchQuery])
  ...
  const handleExportSelected = () => {
    ...
    const selectedData = reviewData.filter((row) => selectedRows.includes(row.id))

    const exportData = selectedData.map((row) => ({
      'Reference ID': `REV-${row.referenceId.toString().padStart(6, '0')}`,
      Employee: row.employee,
      Department: row.department,
      Particulars: row.particulars,
      Amount: `₱${row.amount.toLocaleString()}`,
      Status: row.status,
      'Submission Date': row.liquidationDate
      ...
      'Review Priority': row.reviewPriority,
      Reviewer: row.reviewer || 'Not assigned',
      'Review Deadline': row.reviewDeadline
        ? new Date(row.reviewDeadline).toLocaleDateString()
        : '-',
      ...
      <div className="flex-1 min-h-0 p-3">
        ...
        <div className="bg-component shadow-lg rounded-lg border border-slate-400 mb-3">
          <div className="px-4 py-1">
            <h1 className="text-2xl font-bold text-gray-800">Review Liquidations</h1>
            <p className="text-gray-600">Review pending liquidation requests for approval</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-4 pb-2 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {Array.from(new Set(reviewData.map((item) => item.department))).map((dept) => (
        ...
        <div className="h-[calc(100vh-480px)] lg:h-[calc(100vh-450px)] xl:h-[calc(100vh-360px)] overflow-hidden">
          <div className="bg-component shadow-lg rounded-lg border border-slate-400 h-full flex flex-col p-2">
            ...
            <PlatformTable
              columns={reviewLiquidationsColumns}
...
export default ReviewLiquidations
