...
const myLiquidationsColumns = [
  {
    ...
    render: (value) => (
      <span className="font-mono font-semibold text-blue-700">
        LIQ-{value.toString().padStart(6, '0')}
      </span>
    ),
  },
  {
...
const createMyLiquidationsData = (count) => {
  const employees = [
    'Juan Dela Cruz',
    'Maria Santos',
    'John Smith',
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      receiptStatus: ['Pending', 'Submitted', 'Incomplete'][Math.floor(Math.random() * 3)],
      approvalStatus: ['Awaiting Review', 'Under Verification', 'Pending Approval'][
      ...
      deadlineDate: generateRandomDate(14),
    }
  })
}
...
const MyLiquidations = () => {
  ...
  const liquidationsData = useMemo(() => createMyLiquidationsData(25), [])

  //   filter
  const filteredAndSortedData = useMemo(() => {
    let filtered = [...liquidationsData]
    ...
    if (typeFilter) {
      filtered = filtered.filter((item) => item.liquidationType === typeFilter)
    }
    ...
    return filtered.sort((a, b) => {
      if (sortKey === 'liquidationDate' || sortKey === 'deadlineDate') {
  ...
  }, [liquidationsData, sortKey, sortDirection, departmentFilter, typeFilter, searchQuery])
  ...
  const handleExportSelected = () => {
    ...
    const selectedData = liquidationsData.filter((row) => selectedRows.includes(row.id))

    const exportData = selectedData.map((row) => ({
      'Reference ID': `LIQ-${row.referenceId.toString().padStart(6, '0')}`,
      ...
      'Receipt Status': row.receiptStatus,
      'Approval Status': row.approvalStatus,
      'Deadline Date': row.deadlineDate ? new Date(row.deadlineDate).toLocaleDateString() : '-',
      Notes: row.notes,
    }))
    ...
    link.setAttribute('download', `my-liquidations-${new Date().toISOString().split('T')[0]}.csv`)
      ...
      <div className="flex-1 min-h-0 p-3">
        ...
        <div className="bg-component shadow-lg rounded-lg border border-slate-400 mb-3">
          <div className="px-4 py-1">
            <h1 className="text-2xl font-bold text-gray-800">My Liquidations</h1>
            <p className="text-gray-600">Manage your pending liquidation requests</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-4 pb-2 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {Array.from(new Set(liquidationsData.map((item) => item.department))).map(
                  (dept) => (
              ...
              >
                <option value="">All Liquidation Types</option>
                {Array.from(new Set(liquidationsData.map((item) => item.liquidationType))).map(
                  (type) => (
            ...
            <div className="flex items-center">
              <input
                type="text"
                placeholder="Search by ID, employee, department, particulars, or liquidation type..."
