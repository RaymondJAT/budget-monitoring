...
const verifiedLiquidationsColumns = [
  {
    key: 'referenceId',
    label: 'Reference ID',
    sortable: true,
    ...
    render: (value) => (
      <span className="font-mono font-semibold text-blue-700">
        VER-{value.toString().padStart(6, '0')}
      </span>
    ),
  },
  {
...
const createVerifiedLiquidationsData = (count) => {
  const employees = [
    'Juan Dela Cruz',
    'Maria Santos',
    'John Smith',
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      auditStatus: ['Passed', 'Minor Issues', 'No Issues'][Math.floor(Math.random() * 3)],
      complianceStatus: ['Fully Compliant', 'Partially Compliant', 'Compliant'][
        Math.floor(Math.random() * 3)
      ],
      approvalChain: [
        'Approver 1 → Verifier 1',
        'Approver 2 → Verifier 2',
        'Approver 3 → Verifier 3',
      ][Math.floor(Math.random() * 3)],
      notes: [
      ...
      closureDate: Math.random() > 0.5 ? generateRandomDate(3) : null,
    }
  })
}

const VerifiedLiquidations = () => {
  ...
  const verifiedData = useMemo(() => createVerifiedLiquidationsData(35), [])
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      if (sortKey === 'liquidationDate' || sortKey === 'closureDate') {
  ...
  }, [verifiedData, sortKey, sortDirection, departmentFilter, typeFilter, searchQuery])
  ...
  const handleExportSelected = () => {
    ...
    const exportData = selectedData.map((row) => ({
      'Reference ID': `VER-${row.referenceId.toString().padStart(6, '0')}`,
      ...
      'Verification Date': row.liquidationDate
      ...
      'Approval Chain': row.approvalChain,
      'Closure Date': row.closureDate ? new Date(row.closureDate).toLocaleDateString() : '-',
      Notes: row.notes,
    }))
      ...
      <div className="flex-1 min-h-0 p-3">
        ...
        <div className="bg-component shadow-lg rounded-lg border border-slate-400 mb-3">
          <div className="px-4 py-1">
            ...
            <p className="text-gray-600">View verified and audited liquidation requests</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-4 pb-2 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {Array.from(new Set(verifiedData.map((item) => item.liquidationType))).map(
                  (type) => (
        ...
        <div className="h-[calc(100vh-480px)] lg:h-[calc(100vh-450px)] xl:h-[calc(100vh-360px)] overflow-hidden">
          <div className="bg-component shadow-lg rounded-lg border border-slate-400 h-full flex flex-col p-2">
            ...
            <PlatformTable
              columns={verifiedLiquidationsColumns}
...
export default VerifiedLiquidations
