  ...
  },
  {
    ...
    render: (value) => {
      const statusConfig = {
        ...
        'For Review': { color: 'bg-amber-100 text-amber-800', border: 'border-amber-200' },
        'Under Audit': { color: 'bg-blue-100 text-blue-800', border: 'border-blue-200' },
      }
...
const createForLiquidationData = (count) => {
  const employees = [
    'Juan Dela Cruz',
    'Maria Santos',
    'John Smith',
  ...
  const statuses = ['Completed', 'Verified', 'For Review', 'Under Audit']
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const completionDate =
      status === 'Completed' || status === 'Verified' ? generateRandomDate(30) : null

    return {
      id: i + 1,
      referenceId: 200000 + i + 1,
      ...
      verifiedBy: status === 'Verified' ? `Verifier ${Math.floor(Math.random() * 5) + 1}` : '',
      verifiedDate: status === 'Verified' ? generateRandomDate(7) : '',
      auditor: status === 'Under Audit' ? `Auditor ${Math.floor(Math.random() * 3) + 1}` : '',
      remarks: status === 'For Review' ? 'Pending supporting documents' : '',
    }
  })
}
...
const ForLiquidation = () => {
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      if (sortKey === 'requestDate' || sortKey === 'completionDate' || sortKey === 'verifiedDate') {
  ...
  }, [liquidationData, sortKey, sortDirection, statusFilter, departmentFilter, searchQuery])
  ...
  const handleExportSelected = () => {
    ...
    const selectedData = liquidationData.filter((row) => selectedRows.includes(row.id))
    ...
    link.setAttribute(
      'download',
      `liquidation-selected-${new Date().toISOString().split('T')[0]}.csv`
    )
      ...
      <div className="flex-1 min-h-0 p-3">
        ...
        <div className="bg-component shadow-lg rounded-lg border border-slate-400 mb-3">
          <div className="px-4 py-1">
            <h1 className="text-2xl font-bold text-gray-800">For Liquidation</h1>
            <p className="text-gray-600">
              Manage completed and verified requests ready for liquidation
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-4 pb-2 gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {Array.from(new Set(liquidationData.map((item) => item.department))).map((dept) => (
